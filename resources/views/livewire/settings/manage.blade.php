<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-3xl card-shadow overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-[#f8fafa]">
            <h2 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Logo ລະບົບ</h2>
        </div>
        ...
        <div class="px-6 py-6 space-y-5">
            ...
            @if($currentLogo)
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 w-20 h-20 rounded-2xl border border-gray-200 bg-[#f8fafa] flex items-center justify-center overflow-hidden">
                        <img src="{{ asset('storage/' . $currentLogo) }}" alt="Logo" class="w-full h-full object-contain p-1">
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-slate-800">Logo ປັດຈຸບັນ</p>
                        <p class="text-xs text-gray-400 mt-0.5 break-all">{{ basename($currentLogo) }}</p>
            ...
            @else
                <div class="flex items-center gap-3 p-3 bg-[#f8fafa] rounded-xl border border-gray-200">
                    <span class="text-2xl select-none">☸️</span>
                    <p class="text-sm text-gray-400">ກຳລັງໃຊ້ icon ເລີ່ມຕົ້ນ (☸️) — ອັບໂຫລດ logo ເພື່ອປ່ຽນ</p>
                </div>
            @endif
            ...
            <form wire:submit="save" class="space-y-4">
                ...
                <div>
                    ...
                    <div x-data="{ dragging: false }" @dragover.prevent="dragging = true" @dragleave.prevent="dragging = false"
                        ...
                        <div class="flex flex-col items-center justify-center py-8 px-4 text-center pointer-events-none">
                            ...
                            <p class="text-xs text-gray-400/70 mt-1">PNG, JPG, SVG, GIF — ສູງສຸດ 2MB</p>
                        </div>
                    </div>
                ...
                @if($logo)
                    <div class="flex items-center gap-3 p-3 bg-[#f8fafa] rounded-xl border border-gray-200">
                        <img src="{{ $logo->temporaryUrl() }}" alt="Preview" class="w-12 h-12 object-contain rounded-lg border border-gray-200">
                        <div class="min-w-0">
                            <p class="text-xs font-medium text-slate-800">{{ $logo->getClientOriginalName() }}</p>
                            <p class="text-xs text-gray-400">{{ number_format($logo->getSize() / 1024, 1) }} KB</p>
                        </div>
                    </div>
                @endif
